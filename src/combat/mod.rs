...
pub fn battle_start<'a>(
    mut players: Vec<Hero<'a>>,
    mut baddies: Vec<Mob>, /*slice filled with encounter data*/
) -> BattleResult<'a> {
    const DMG_HURT: u8 = 1;
    ...
    let mut enable_player_commands: bool = true;
    let mut enemy_turn_count: usize = 0;
    let mut player_turn_count: usize = 0;
    ...
    loop {
        ...
        loop {
            ...
            // Player turn
            if players[player_turn_count].stats.dexterity + players[player_turn_count].stats.level
                > baddies[enemy_turn_count].stats.dexterity + baddies[enemy_turn_count].stats.level
            {
                player_turn_count += 1; // Increment player turn count regardless
                        ...
                        loop {
                            ...
                            match command {
                                1 => {
                                    ...
                                    attack =
                                        generate_weapon_attack(players[counter].equipment.weapon);
                                    break;
                                }
            ...
            else if enemy_turn_count > baddies.len() && player_turn_count > players.len() {
                // Reset player turn length
                player_turn_count = 0;
                enemy_turn_count = 0
            } else {
                println!("\nEnemy Turn\n");
                for counter in &mut baddies {
                    let playerlen: usize = players.len(); // Used to work around a borrow limit
                    let target: &mut Hero<'a> =
                        &mut players[rand::thread_rng().gen_range(0..playerlen)];
                    ...
                    if counter.hp > 0 && target.hp > 0 {
                        ...
                        loop {
                            attack = counter.movelist
                                [rand::thread_rng().gen_range(0..counter.movelist.len())];
...
fn calculate_damage(
    ...
    user_stats: Stats,
    target_stats: Stats,
    target_elements: ElementalEffects,
) -> u16 {
